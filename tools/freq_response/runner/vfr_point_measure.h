...
typedef enum {
    ...
    VFR_POINT_SAMPLE_LIMIT_EXCEEDED, /**< Requested measurement samples exceed the per-point limit. */
...
typedef struct {
    ...
    uint32_t max_samples_per_point; /**< Maximum synchronous-detection samples per point. */
...
typedef struct {
    ...
    uint32_t warmup_samples;      /**< Number of pre-measurement settling samples executed. */
    uint32_t measure_samples;     /**< Number of samples used for synchronous detection. */
    uint32_t total_samples;       /**< Total executed samples: warmup_samples + measure_samples. */
 ...
 *
 * Warmup samples settle the subject and are not included in the synchronous
 * detection / single-frequency DFT accumulation. max_samples_per_point limits
 * only the measurement samples used by that accumulation. The phase convention
 * is output phase minus input phase, so output lead is positive and output lag
 * is negative.
 */
